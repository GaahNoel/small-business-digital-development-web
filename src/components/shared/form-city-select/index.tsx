...
export const FormCitySelect = ({
  ...
  const onChange: SubmitHandler<CityFormData> = async ({ city }) => {
    ...
    <>
      <FormControl as="form" onChange={handleSubmit(onChange)}>
        <Stack
          ...
          direction="row"
        >
          <Select bg="default_white" {...register('city')}>
            <option value="Selecione uma cidade" disabled={citySelected}>
              ...
              cityOptions.map((location, keyState) => (
                <optgroup key={keyState} label={location.state}>
                  {location.cities.map((city, key) => (
                    <option key={key} value={`${city}-${location.state}`}>
                      {city}
                    </option>
                  ))}
                </optgroup>
